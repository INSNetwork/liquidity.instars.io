...
export function useProvideLiquidity() {
  const antContract = useKnownContract('TOKEN_INSTAR')
...
export function useApproveINSTAR() {
  const uniContract = useKnownContract('TOKEN_INSTAR')
  const [unipoolAddress] = getKnownContract('UNIPOOL')
  const getAllowance = useAllowance()
...
export function useNotifyRewards() {
  const unipoolContract = useKnownContract('UNIPOOL');
  const getApproval = useApproveINSTAR();
  return useCallback(
    async amount => {
      try {
        if (!unipoolContract) {
