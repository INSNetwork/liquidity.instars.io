...
export function useInstarUniswapInfo() {
  ...
  useEffect(() => {
    ...
    async function fetchTokenInfo() {
      try {
        setLoading(true)
        const [results, totalSupply, totalStaked, totalWallets, cryptoResult] = await Promise.all([
          ...
          fetch('https://crypto.com/price/coin-data/insights-network/basic_info.json').then(response => response.json())
        ]);
