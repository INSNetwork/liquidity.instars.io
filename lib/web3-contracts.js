...
function getUniswapPairQuery(pair) {
  return `
  query {
    pairs(where: { id: "${pair}" }) {
      id
...
function getUniswapTokenQuery() {
  return `
  query {
    token(id: "0x8193711b2763bc7dfd67da0d6c8c26642eafdaf3"){
      name
      symbol
      decimals
      derivedETH
    ...
    tokenDayDatas(orderBy: date, orderDirection: desc, first:1,
      where: {
          token: "0x8193711b2763bc7dfd67da0d6c8c26642eafdaf3"
      }
      ) {
          id
          date
  ...
  useEffect(() => {
    ...
    async function fetchTokenInfo() {
      try {
        ...
        const [tokenUniAddress] = getKnownContract(pair, 'TOKEN_UNI');
        if (!tokenAddress) {
        ...
        const opts = {
          method: 'POST',
          body: JSON.stringify({ query: getUniswapPairQuery(tokenUniAddress)}),
        }
...
export function useInstarUniswapInfo() {
  const [tokenInfo, setTokenInfo] = useState(null)
  const [loading, setLoading] = useState(false)
  useEffect(() => {
    let cancelled = false
    ...
    async function fetchTokenInfo() {
      try {
        setLoading(true)
        const [results, totalSupply, totalStaked, totalWallets, coincapResult] = await Promise.all([
          await fetch(UNISWAP_SUBGRAPH, {
            method: 'POST',
            body: JSON.stringify({ query: getUniswapTokenQuery()}),
          }).then(response => response.json()),
          fetch('https://wallet-service.instars.com/total_supply').then(response => response.text()),
          fetch('https://wallet-service.instars.com/total_staked').then(response => response.text()),
          fetch('https://lightapi.eosamsterdam.net/api/usercount/instar').then(response => response.text()),
          fetch('https://api.coincap.io/v2/assets/insights-network').then(response => response.json())
        ]);
       
        if (!cancelled) {
          setTokenInfo({
            token: results?.data?.token,
            dayData: results?.data?.tokenDayDatas[0],
            totalSupply: totalSupply,
            totalStaked: totalStaked,
            totalWallets: totalWallets,
            coincapResult: coincapResult
          })
          setLoading(false)
        }
      } catch (e) {
